"""PowerPoint Generator - Template override fixed"""
...
class EnhancedPPTGenerator:
    ...
    def __init__(self, template_path=None):
        if template_path and Path(template_path).exists():
            try:
                ...
                logger.info("Cleared template slides")
    ...
    def _handle_table_layout(self, slide, slide_data):
        ...
        for ri, row in enumerate(data, 1):
            for ci, val in enumerate(row if isinstance(row, (list, tuple)) else [row]):
                if ci < cols:
                    ppt_table.cell(ri, ci).text = str(val) if val is not None else ""
    ...
    def _handle_metrics_layout(self, slide, slide_data):
        content = slide_data.get('content', {})
        metrics = content.get('key_metrics', []) or []
        if not metrics:
            return self._handle_content_layout(slide, slide_data)
        if len(slide.shapes) > 1:
            slide.shapes._spTree.remove(slide.shapes[1]._element)
    ...
    def _handle_timeline_layout(self, slide, slide_data):
        content = slide_data.get('content', {})
        items = content.get('timeline_items', []) or []
        if not items:
            return self._handle_content_layout(slide, slide_data)
        if len(slide.shapes) > 1:
            slide.shapes._spTree.remove(slide.shapes[1]._element)
        ...
        for i, it in enumerate(items):
            ...
            if it.get('description'):
                ...
                db.text_frame.paragraphs[0].text = it['description']
    ...
    def _handle_image_layout(self, slide, slide_data):
        ...
        try:
            if img.startswith('http'):
                self._add_placeholder(slide, lp, tp, iw, ih, "URL not supported")
                return
            if img.startswith('image'):
                ...
                return
            self._add_placeholder(slide, lp, tp, iw, ih, "Not found")
        except Exception as e:
            logger.warning(f"Image: {e}")
            self._add_placeholder(slide, lp, tp, iw, ih, str(e))

    def _add_placeholder(self, slide, left, top, width, height, reason):
